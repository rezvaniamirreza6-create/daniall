...
async def admin_cat_title(message: Message, state: FSMContext):
    ...
    await message.answer("یه ایموجی برای این دسته بفرست (مثلاً 🎁):")
...
async def cb_admin_cat_edit_title(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("نام جدید دسته رو بفرست:")
    await call.answer()
...
async def cb_admin_cat_edit_emoji(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("ایموجی جدید رو بفرست:")
    await call.answer()
...
async def cb_admin_prod_add(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("نام محصول جدید رو بفرست (مثلاً «پرمیوم ۳ ماهه»):")
    await call.answer()
...
async def admin_prod_title(message: Message, state: FSMContext):
    ...
    await message.answer("قیمت این محصول رو به تومان بفرست (فقط عدد):")
...
async def admin_prod_price(message: Message, state: FSMContext):
    ...
    await message.answer("توضیحات محصول رو بفرست (یا بنویس «-» برای رد کردن):")
...
async def cb_admin_prod_edit_prompt(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("متن جدید سوال رو بفرست:")
    await call.answer()
...
async def cb_admin_prod_edit_title(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("نام جدید محصول رو بفرست:")
    await call.answer()
...
async def cb_admin_prod_edit_price(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("قیمت جدید رو به تومان بفرست (فقط عدد):")
    await call.answer()
...
async def cb_admin_prod_edit_desc(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("توضیحات جدید رو بفرست:")
    await call.answer()
...
async def cb_admin_admin_add(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("آیدی عددی کاربر موردنظر برای افزودن به ادمین‌ها رو بفرست:")
    await call.answer()
...
async def cb_admin_admin_remove(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminManage.waiting_admin_id)
    await call.message.answer("آیدی عددی ادمینی که می‌خوای حذف کنی رو بفرست:")
    await call.answer()
...
async def cb_admin_user_balance(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("آیدی عددی کاربر موردنظر رو بفرست:")
    await call.answer()
...
async def cb_admin_set(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"مقدار فعلی: <code>{current}</code>\n\n{SETTING_PROMPTS[key]}", parse_mode="HTML")
    await call.answer()
...
async def cb_admin_broadcast(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("پیامی که می‌خوای برای همه کاربران ارسال بشه رو بفرست:")
    await call.answer()
