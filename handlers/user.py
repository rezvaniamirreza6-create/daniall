...
def normalize_iran_phone(raw: str):
    digits = re.sub(r"[^\d+]", "", raw)
    if not IRAN_PHONE_RE.match(digits):
        return None
    only = re.sub(r"\D", "", digits)
    if only.startswith("0098"):
        only = only[4:]
    elif only.startswith("98"):
        only = only[2:]
    elif only.startswith("0"):
        only = only[1:]
    return "+98" + only


# ---------------- دکمه بازگشت سراسری ----------------
# این هندلر باید همیشه اول از همه چک بشه تا کاربر در هر مرحله‌ای بتونه لغو کنه
@router.message(F.text == BACK_TEXT)
async def global_back(message: Message, state: FSMContext):
    await state.clear()
    user = await db.get_user(message.from_user.id)
    if not user or not user["is_verified"]:
        await message.answer("برای ادامه، شماره تلفنت رو تایید کن:", reply_markup=kb.phone_request_kb())
        return
    is_adm = await db.is_admin(message.from_user.id, config.OWNER_ID)
    await message.answer("به منوی اصلی برگشتی 👇", reply_markup=kb.main_menu_kb(is_adm))
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not user["is_verified"]:
        await message.answer(
            "👋 خوش اومدی!\n\n"
            "برای استفاده از ربات، ابتدا باید شماره تلفن همراه ایرانی‌ت رو با دکمه‌ی زیر ارسال کنی:",
...
async def receive_contact(message: Message, state: FSMContext):
    ...
    if contact.user_id and contact.user_id != message.from_user.id:
        await message.answer(
            "❗️ لطفاً فقط شماره‌ی تلفن خودت رو با دکمه‌ی زیر ارسال کن (نه شخص دیگه‌ای).",
            reply_markup=kb.phone_request_kb(),
        )
        return
    ...
    if not normalized:
        await message.answer(
            "❌ فقط شماره تلفن همراه ایرانی (مثل 0912xxxxxxx) قابل قبوله. دوباره امتحان کن:",
            reply_markup=kb.phone_request_kb(),
        )
        return

    await db.verify_user_phone(message.from_user.id, normalized)
    await message.answer("✅ شماره‌ت با موفقیت تایید شد!")
...
async def require_verified(message: Message) -> bool:
    user = await db.get_user(message.from_user.id)
    if not user or not user["is_verified"]:
        await message.answer("برای ادامه، اول شماره تلفنت رو تایید کن:", reply_markup=kb.phone_request_kb())
        return False
    if user["is_banned"]:
        await message.answer("⛔️ شما توسط ادمین مسدود شده‌اید.")
        return False
...
async def cb_open_category(call: CallbackQuery, state: FSMContext):
    ...
    if cat["kind"] == "currency":
        p = products[0]
        await state.update_data(currency_product_id=p["id"])
        await state.set_state(BuyFlow.waiting_currency_amount)
        await call.message.answer(
            f"💱 <b>{cat['title']}</b>\n"
            f"نرخ هر واحد: {fmt_money(p['price'])}\n\n"
            f"مقدار ارز موردنظرت رو به عدد بفرست (مثلاً 10):",
            reply_markup=kb.back_kb(), parse_mode="HTML",
        )
        await call.answer()
        return
...
async def receive_target(message: Message, state: FSMContext):
    ...
    if not prod or not prod["is_active"]:
        await state.clear()
        is_adm = await db.is_admin(message.from_user.id, config.OWNER_ID)
        await message.answer("این محصول دیگر موجود نیست.", reply_markup=kb.main_menu_kb(is_adm))
        return
    ...
    await message.answer(text, reply_markup=kb.main_menu_kb(is_adm), parse_mode="HTML")
    await message.answer("برای تایید نهایی دکمه زیر رو بزن:", reply_markup=kb.confirm_buy_kb(prod["id"]))


@router.message(BuyFlow.waiting_currency_amount)
async def receive_currency_amount(message: Message, state: FSMContext):
    if not message.text.replace(".", "", 1).isdigit():
        await message.answer("لطفاً فقط عدد بفرست (مثلاً 10).", reply_markup=kb.back_kb())
        return

    data = await state.get_data()
    prod = await db.get_product(data["currency_product_id"])
    if not prod or not prod["is_active"]:
        await state.clear()
        is_adm = await db.is_admin(message.from_user.id, config.OWNER_ID)
        await message.answer("این محصول دیگر موجود نیست.", reply_markup=kb.main_menu_kb(is_adm))
    ...
    text = (
        f"💱 <b>{prod['title']}</b>\n"
        f"مقدار: {amount}\n"
        f"نرخ واحد: {fmt_money(prod['price'])}\n"
        f"مبلغ کل: <b>{fmt_money(total)}</b>\n\n"
        f"تایید می‌کنی؟"
    )
    b = InlineKeyboardBuilder()
    b.button(text="✅ تایید و پرداخت از کیف پول", callback_data=f"confirm_currency:{prod['id']}:{amount}")
    b.button(text="❌ انصراف", callback_data="cancel_buy")
    b.adjust(1)
    await message.answer(text, reply_markup=kb.main_menu_kb(is_adm), parse_mode="HTML")
    await message.answer("برای تایید نهایی دکمه زیر رو بزن:", reply_markup=b.as_markup())
...
async def cb_confirm_buy(call: CallbackQuery, state: FSMContext):
    ...
    if stock_content:
        ...
        await call.message.edit_text(
            ...
            f"⏳ ادمین به‌زودی به‌صورت دستی سفارشتون رو تامین و ارسال می‌کنه."
        )
        await notify_admins(
            call.bot,
            f"🆕 سفارش جدید نیازمند تامین دستی #{order_id}\n"
            f"محصول: {prod['title']}\n"
            f"کاربر: {user['user_id']} (@{call.from_user.username or '-'})\n"
            + (f"اطلاعات ارسالی کاربر: {target_info}\n" if target_info else "")
            + f"مبلغ: {fmt_money(prod['price'])}\n"
            f"لطفاً سفارش رو دستی تحویل بدید.",
        )
    await call.answer()
...
async def cb_topup_method(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"مبلغ موردنظر برای شارژ رو به تومان وارد کن (حداقل {int(min_amt):,} تومان):",
...
async def topup_amount(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("لطفاً فقط عدد وارد کن (مثلاً 50000).", reply_markup=kb.back_kb())
        return
    amount = int(message.text)
    min_amt = int(await db.get_setting("min_topup") or 0)
    if amount < min_amt:
        await message.answer(f"حداقل مبلغ شارژ {min_amt:,} تومان است. عدد دیگه‌ای بفرست:", reply_markup=kb.back_kb())
        return
...
async def topup_receipt_invalid(message: Message):
    await message.answer("لطفاً عکس رسید پرداخت رو ارسال کن.", reply_markup=kb.back_kb())
...
async def account_info(message: Message):
    ...
    await message.answer(
        ...
        f"شماره تایید‌شده: <code>{user['phone']}</code>\n"
...
async def support_info(message: Message):
    if not await require_verified(message):
        return
    support = await db.get_setting("support_contact")
